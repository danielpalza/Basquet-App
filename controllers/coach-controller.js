    ...
    if (coach) {
      ...
      if (isOk) {
        const token = jwt.sign(
          { _id:coach._id ,firstName: coach.firstName, lastName: coach.lastName, email: coach.email},
...
const createCoach = async (req, res) => {
  try {
    ...
    if (e.code && e.code === 11000) {
      res.status(400).send({ status: 'DUPLICATED', message: "El email ya esta registrado" });
    } else {
